...
--
-- 枚举编码规范：所有状态/类型字段统一使用 TINYINT 数字编码，COMMENT 中注明映射关系。
...
CREATE TABLE IF NOT EXISTS `football_player` (
  ...
  `position`        TINYINT      NOT NULL COMMENT '0=守门员,1=后卫,2=中场,3=前锋',
  `nationality`     VARCHAR(32)  NOT NULL,
  `birth_date`      DATE         DEFAULT NULL,
  `height`          DECIMAL(3,2) DEFAULT NULL COMMENT '身高（米）',
  `preferred_foot`  TINYINT      DEFAULT NULL COMMENT '0=左脚,1=右脚,2=双脚',
  `status`          TINYINT      NOT NULL DEFAULT 0 COMMENT '0=活跃,1=非活跃',
  ...
  CONSTRAINT `ck_player_position` CHECK (`position` IN (0,1,2,3)),
  CONSTRAINT `ck_player_status` CHECK (`status` IN (0,1)),
...
CREATE TABLE IF NOT EXISTS `football_player_contract` (
  ...
  `contract_type`     TINYINT       NOT NULL COMMENT '0=标准合同,1=青年合同,2=短期合同',
  ...
  `status`            TINYINT       NOT NULL COMMENT '0=草拟中,1=已签署,2=已到期,3=已终止',
  `create_by`         VARCHAR(64)   NOT NULL DEFAULT 'system',
  `create_time`       DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_by`         VARCHAR(64)   DEFAULT NULL,
  `update_time`       DATETIME      DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  ...
  CONSTRAINT `ck_contract_type` CHECK (`contract_type` IN (0,1,2)),
  CONSTRAINT `ck_contract_status` CHECK (`status` IN (0,1,2,3))
...
CREATE TABLE IF NOT EXISTS `football_match` (
  ...
  CONSTRAINT `ck_match_scores` CHECK (
    (`status` = 1 AND `home_score` IS NOT NULL AND `away_score` IS NOT NULL) OR
    (`status` <> 1 AND `home_score` IS NULL AND `away_score` IS NULL)
  )
...
CREATE TABLE IF NOT EXISTS `football_training` (
  ...
  `training_type`   TINYINT      NOT NULL COMMENT '0=体能,1=战术,2=技术,3=恢复,4=热身',
  `start_time`      DATETIME     NOT NULL,
  `end_time`        DATETIME     NOT NULL,
  `venue`           VARCHAR(128) DEFAULT NULL,
  `description`     TEXT         DEFAULT NULL,
  `training_goal`   TEXT         DEFAULT NULL,
  `status`          TINYINT      NOT NULL COMMENT '0=已计划,1=已完成,2=已取消',
  `create_by`        VARCHAR(64)  NOT NULL DEFAULT 'system',
  `create_time`      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_by`        VARCHAR(64)  DEFAULT NULL,
  `update_time`      DATETIME     DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  ...
  CONSTRAINT `ck_training_type` CHECK (`training_type` IN (0,1,2,3,4)),
  CONSTRAINT `ck_training_status` CHECK (`status` IN (0,1,2))
...
CREATE TABLE IF NOT EXISTS `football_training_player` (
  ...
  `player_id`         BIGINT      NOT NULL,
  `attendance_status` TINYINT     NOT NULL DEFAULT 0 COMMENT '0=待确认,1=已出勤,2=缺勤,3=请假',
  `create_time`       DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `ck_attendance_status` CHECK (`attendance_status` IN (0,1,2,3))
...
CREATE TABLE IF NOT EXISTS `football_injury` (
  ...
  `recovery_status`      TINYINT      NOT NULL COMMENT '0=未知,1=治疗中,2=康复中,3=已康复',
  ...
  CONSTRAINT `ck_injury_status` CHECK (`recovery_status` IN (0,1,2,3))
...
VALUES
(1,21,'齐雨熙','Qi Yuxi',0,'中国','2006-07-18',NULL,NULL,0,NULL,'https://match.sports.sina.com.cn/football/csl/team.php?dpc=1&id=148','2026-08-25'),
(2,25,'闫炳良','Yan Bingliang',0,'中国','2000-04-03',1.97,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(3,26,'张皓然','Zhang Haoran',0,'中国',NULL,NULL,NULL,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(4,3,'王政豪','Wang Zhenghao',1,'中国',NULL,NULL,NULL,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(5,4,'杨帆','Yang Fan',1,'中国','1996-03-28',1.82,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(6,6,'王献钧','Wang Xianjun',1,'中国','2000-06-01',1.88,0,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(7,17,'吴兴涵','Wu Xinghan',1,'中国','1993-02-24',1.83,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(8,18,'艾托尔·科尔多瓦','Aitor Cordoba',1,'西班牙','1995-05-21',1.91,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(9,27,'李嗣镕','Li Silong',1,'中国',NULL,NULL,NULL,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(10,31,'孙铭谦','Sun Minghim',1,'中国香港',NULL,NULL,NULL,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(11,38,'李帅琪','Li Shuaiqi',1,'中国','2008-09-08',1.90,1,0,NULL,'https://m.zhibo8.cc/news/web/zuqiu/2026-02-28/69a2d65033a76native.htm','2026-02-28'),
(12,39,'蔡承峻','Cai Chengjun',1,'中国','2005-01-31',1.83,1,0,NULL,'https://m.zhibo8.cc/news/web/zuqiu/2026-02-28/69a2d65033a76native.htm','2026-02-28'),
(13,40,'石炎','Shi Yan',1,'中国',NULL,NULL,NULL,0,'公开名单同时也有将其列为前锋的资料；此处按2026-07-24阵容页','https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(14,5,'豪梅·格劳','Jaume Grau',2,'西班牙','1997-05-05',1.84,0,1,'2026赛季上半程注册，夏窗更新名单中被萨尔瓦多替换','https://www.qtx.com/csl/279129.html','2026-08-10'),
(15,8,'布鲁诺·哈达斯','Bruno Xadas',2,'葡萄牙','1997-12-02',1.79,0,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(16,10,'克里斯蒂安·萨尔瓦多','Cristian Salvador',2,'西班牙','1994-11-20',1.84,NULL,0,'冬窗因踝关节韧带手术暂未报名，夏窗回归更新名单','https://www.leisu.com/data/zuqiu/player-88945','2026-07-24'),
(17,14,'黄嘉辉','Huang Jiahui',2,'中国','2000-10-07',1.85,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(18,16,'刘帅','Liu Shuai',2,'中国','2004-11-26',1.79,1,0,NULL,'https://m.zhibo8.cc/news/web/zuqiu/2026-02-28/69a2d65033a76native.htm','2026-02-28'),
(19,20,'季胜攀','Ji Shengpan',2,'中国','1999-11-08',1.78,NULL,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(20,22,'李永佳','Li Yongjia',2,'中国',NULL,NULL,NULL,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(21,24,'陈哲宣','Chen Zhexuan',2,'中国','2003-09-24',1.81,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(22,28,'郭皓','Guo Hao',2,'中国',NULL,NULL,NULL,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(23,29,'巴顿','Ba Dun',2,'中国','1995-09-16',1.81,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(24,30,'王秋明','Wang Qiuming',2,'中国','1993-01-09',1.73,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(25,33,'乃博宁林','Naiboninglin',2,'中国','2006-03-13',1.86,1,0,NULL,'https://m.zhibo8.cc/news/web/zuqiu/2026-02-28/69a2d65033a76native.htm','2026-02-28'),
(26,7,'吉列尔梅·谢蒂内','Guilherme Schettine',3,'巴西','1995-10-10',1.80,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(27,9,'阿尔韦托·基莱斯','Alberto Quiles',3,'西班牙','1995-04-27',1.88,0,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(28,11,'谢维军','Xie Weijun',3,'中国','1997-11-14',1.90,1,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24'),
(29,19,'刘俊贤','Liu Junxian',3,'中国',NULL,NULL,NULL,0,NULL,'https://www.qiumiwu.com/team/tianjinjinmenhu/roster','2026-07-24')
ON DUPLICATE KEY UPDATE
...
VALUES
(1,'2026',1,'2026-03-07 15:30:00','天津津门虎','重庆铜梁龙',0,0,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(2,'2026',2,'2026-03-14 20:00:00','深圳新鹏城','天津津门虎',1,0,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(3,'2026',3,'2026-03-21 15:30:00','辽宁铁人楠波湾','天津津门虎',3,0,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(4,'2026',4,'2026-04-05 19:35:00','天津津门虎','上海申花',2,3,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(5,'2026',5,'2026-04-12 19:00:00','天津津门虎','青岛海牛',1,1,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(6,'2026',6,'2026-04-17 20:00:00','云南玉昆','天津津门虎',0,3,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(7,'2026',7,'2026-04-21 20:00:00','天津津门虎','山东泰山',1,2,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(8,'2026',8,'2026-04-25 19:35:00','北京国安','天津津门虎',2,4,1,'北京工人体育场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(9,'2026',9,'2026-05-01 19:35:00','天津津门虎','武汉三镇',2,2,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(10,'2026',10,'2026-05-05 19:00:00','青岛西海岸','天津津门虎',1,1,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(11,'2026',11,'2026-05-10 19:35:00','浙江俱乐部绿城','天津津门虎',1,1,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(12,'2026',12,'2026-05-15 19:35:00','天津津门虎','成都蓉城',1,2,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(13,'2026',13,'2026-05-19 19:35:00','天津津门虎','河南俱乐部彩陶坊',1,2,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(14,'2026',14,'2026-05-23 19:00:00','上海海港','天津津门虎',1,1,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(15,'2026',15,'2026-05-31 19:00:00','天津津门虎','大连英博海发',1,0,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(16,'2026',16,'2026-06-27 20:00:00','重庆铜梁龙','天津津门虎',1,0,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(17,'2026',17,'2026-07-04 20:00:00','天津津门虎','深圳新鹏城',3,0,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(18,'2026',18,'2026-09-12 19:00:00','天津津门虎','辽宁铁人楠波湾',NULL,NULL,2,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(19,'2026',19,'2026-07-18 19:35:00','上海申花','天津津门虎',0,2,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(20,'2026',20,'2026-07-25 17:30:00','青岛海牛','天津津门虎',0,2,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(21,'2026',21,'2026-08-01 19:00:00','天津津门虎','云南玉昆',3,2,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(22,'2026',22,'2026-08-09 20:00:00','山东泰山','天津津门虎',2,1,1,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(23,'2026',23,'2026-08-15 19:35:00','天津津门虎','北京国安',2,4,1,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(24,'2026',24,'2026-08-22 19:00:00','武汉三镇','天津津门虎',0,0,1,'武汉体育中心体育场',0,'中国足球超级联赛','https://sports.cctv.com/2026/08/22/ARTIMJRwHuZoOa8OOeJ2Fmhv260822.shtml','2026-08-25'),
(25,'2026',25,'2026-08-29 19:00:00','天津津门虎','青岛西海岸',NULL,NULL,0,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(26,'2026',26,'2026-09-06 20:00:00','天津津门虎','浙江俱乐部绿城',NULL,NULL,0,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(27,'2026',27,'2026-10-10 19:35:00','成都蓉城','天津津门虎',NULL,NULL,0,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(28,'2026',28,'2026-10-16 19:35:00','河南俱乐部彩陶坊','天津津门虎',NULL,NULL,0,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(29,'2026',29,'2026-10-24 15:30:00','天津津门虎','上海海港',NULL,NULL,0,'天津泰达足球场',0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25'),
(30,'2026',30,'2026-11-08 15:30:00','大连英博海发','天津津门虎',NULL,NULL,0,NULL,0,'中国足球超级联赛','https://www.tianjinfc.com/fixtures.html','2026-08-25')
ON DUPLICATE KEY UPDATE
...
VALUES
(31,'2026',4,'2026-06-19 19:30:00','兰州陇原竞技','天津津门虎',1,1,1,'兰州奥体中心玫瑰场',1,'中国足球协会杯',
...
SELECT
  ...
  0, m.match_date, DATE_ADD(m.match_date, INTERVAL 2 HOUR), m.venue,
  CONCAT('公开赛程同步；数据源：', m.source_url),
  CASE m.status WHEN 1 THEN 1 WHEN 2 THEN 3 ELSE 0 END,
  'system'
...
SELECT
  season,
  SUM(CASE WHEN record_type=0 THEN amount ELSE 0 END) AS total_income,
  SUM(CASE WHEN record_type=1 THEN amount ELSE 0 END) AS total_expense,
  SUM(CASE WHEN record_type=0 THEN amount ELSE -amount END) AS net_profit
...
SELECT
  ...
  SUM(tp.attendance_status=1) AS attended_count,
  ROUND(100.0 * SUM(tp.attendance_status=1) / NULLIF(COUNT(*),0), 2) AS attendance_rate
